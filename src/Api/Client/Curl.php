...
use Bluetea\Api\Authentication\Basic;
...
class CurlClient extends BaseClient implements ClientInterface
{
    ...
    public function init()
    {
        ...
        if ($this->authentication instanceof Basic) {
    ...
    protected function finishCall($ch, $data)
    {
        ...
        // Check if no content
        if ($httpStatus === StatusCodes::HTTP_NO_CONTENT || is_null($data)) {
        ...
        if (!StatusCodes::isError($httpStatus)) {
            ...
            throw new HTTPException(sprintf('HTTP error occurred. Got HTTP status code %s', $httpStatus));
        }
    }
